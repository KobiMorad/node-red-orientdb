...
function registerDatabase(NodeRED) {
    function open(node) {
    ...
    function Database(config) {
        ...
        NodeRED.httpAdmin.get('/refresh/:id', function (req, res) {
            ...
            const remoteServer2 = NodeRED.nodes.getNode(req.params.id);
            if (remoteServer2) {
                const remoteServer3 = remoteServer2.getServer();
        ...
        const remoteServer = NodeRED.nodes.getNode(config.server);
        if (remoteServer === null) {
            return '';
        }
        const server = remoteServer.getServer();
